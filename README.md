# Hi, I'm Arya Singh 
📊 Data analyst | Aspiring Data Scientist
🎓 MCA Graduate from Birla Institute of Technology (2024)   
💡 Passionate about transforming data into meaningful insights and impactful visual stories
 - GOOGLE DATA ANALYTICS | IBM | MICROSOFT AI SKILL FEST CERTIFIED | DELOITTE FORAGE DA | GUVI/INFOSIS PYTHON CERTIFIED |        ANTHROPIC COURSES
 - ACTIVE ON KAGGLE/MEDIUM/TWITTER
 -  open to work - Remote / PAN INDIA

## 📊 Tech Stack 
 ## Languages : Python | SQL | MYSQL | HTML | CSS | JAVASCRIPT
 ## ML  & AI  :   Scikit-learn | Linear Regression | Logistic Regression | Agentic AI | AI | Prompt Engineering

## Libraries: Pandas | NumPy | Matplotlib | Seaborn | Plotly

## Tools: Excel |  Git | GitHub | Jupyter Notebook | VS Code | Antigravity IDE

## Concepts: EDA | Feature Engineering | Statistics | Hypothesis Testing | Data Storytelling  | Data Preprocessing 

## Published Articles : 
**Towards AI** — Prompt Engineering: The CRAFT Formula
**Stackademic** — Global Data Science Salary Analysis
  IPL Advanced Analytics — Medium
  My Data Science Journey — Medium

## Currently working on 
- Learning Advance Machine learning
- GSSOC 2026 open source contribution

##  Featured Projects
- Global data science salary analysis
- Customer Churn Analysis
- Data Analytics case studies
- IPL Advance analytics
- Supply Chain Analysis
- AI MULTI AGENT DATA ASSISTANT
- ML CASE STUDIES

##  Areas of Interest
- Data Analytics
- Data Science
- Business Insights
- Visualization & Storytelling
- Machine Learning

##  Goals
- Build impactful real-world data projects
- Strengthen problem-solving skills
- Grow as a Data Analyst / Data Scientist
- 
## 📫 Connect With Me
**GitHub**: https://github.com/Aryasingh06
**Kaggle**: https://www.kaggle.com/aryaksingh
**Medium** : https://medium.com/@singharya0621
**Certificates and internship** : https://drive.google.com/drive/folders/1ro4RTrtxnSTRdcIlia3fzCDRPXGina1f

  ![Arya's GitHub Stats](https://github-readme-stats.vercel.app/api?username=Aryasingh06&show_icons=true&theme=default)
  ![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=Aryasingh06&layout=compact)
  ![GitHub Streak](https://streak-stats.demolab.com?user=Aryasingh06&theme=default)
